...
export default function Dashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true)
        logger.info('UI', 'Dashboard: Starting data load', { userId: user?.id })
        ...
        logger.info('UI', 'Dashboard: Data loaded successfully', {
        ...
        logger.error('UI', `Dashboard: Failed to load data - ${error instanceof Error ? error.message : 'Unknown error'} (User: ${user?.id})`)
